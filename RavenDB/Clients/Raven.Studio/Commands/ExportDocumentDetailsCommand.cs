		...
		private class Exporter
		{
			...
			public async Task ExportAsync(CancellationToken cancellationToken, Action<int> reportProgress)
			{
				...
				using (var writer = new CsvWriter(stream))
				{
					...
					writer.WriteRecord(columns.Select(c => c.Header));
			...
			{
				foreach (var values in documents.Select(extractor.GetValues))
				{
					writer.WriteRecord(values.Select(x => x == null ? "null" : x.ToString()));
				}
			}
		}
	}
